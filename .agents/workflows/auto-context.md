...
activation: Called automatically during /session-offload.md), or manually via /auto-context.md
---
...
For each extracted piece of knowledge.md), determine its proper destination within the 82-file structure.
...
- **Action**: Use your `replace_file_content` or `write_to_file` tools to inject the knowledge.md) into the file.
- **Format Constraint**: If the file contains a `<!-- LEGACY MERGE START -->` or `<!-- CONVERSATION MERGE START -->` marker, insert the new knowledge.md) below it. Use clear bullet points and specify the date of extraction.
...
You MUST execute the Orion Ingestion script immediately after modifying the context files so the Brain Graph indexes the new knowledge.md).
