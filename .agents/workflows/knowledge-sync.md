...
- [ ] Review `MEMORY.md` or `git diff` to understand what was changed in the current session.
...
- [ ] `MEMORY.md` / `HANDOFF.md`: Flush short-term episodic memory into long-term architectural notes, and prepare a clean handoff state.
...
- [ ] Commit with semantic version tag: `git commit -m "chore(docs): sync knowledge base for v<NEW_VERSION>"`
