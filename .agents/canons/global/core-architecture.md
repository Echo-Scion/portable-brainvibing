...
> ⚠️ CRITICAL COUPLING: Modifying the Core Architecture mandates a full ecosystem check. After edits, you MUST run `python .agents/scripts/orion.py verify_agents`.
...
- All major tasks must be broken into atomic tasks using `TASK_PLANNING.template.md`.
- Use "Surgical Munching" (targeted line reading) to minimize context rot.
- Utilize the AutoHarness Protocol (`rules/development-operations.md`) for tasks with complex constraints.

---
*Global Foundation Protocol v0.0.1*
