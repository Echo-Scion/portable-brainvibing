...
> ⚠️ CRITICAL COUPLING: This file is functionally coupled with `scripts/commands/verify_agents.py` and the core ecosystem. If you change the versioning pipeline here, you MUST also verify constraints using `python .agents/scripts/orion.py verify_agents`.
...
python portable\ brainvibing/.agents/scripts/orion.py verify
```
...
git add .
git commit -m "v0.0.1: Decoupled SKILL.md routers"
...
### Scenario
Developer updates `.agents/skills/project-architect/SKILL.md` in `_foundation`
...
cd portable\ brainvibing
git add .
git commit -m "v0.0.1: Enhanced project-architect with RLS patterns"
git push
```
...
- `.agents/skills/project-architect/`: Updated with sanitized content ready for distribution
...
*Last updated: 2026-03-29 | Applies to .agents v0.0.1+*
