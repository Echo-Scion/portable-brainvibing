...
def lint_context(strict: bool = True) -> Tuple[int, int, List[str]]:
    ...
    # Bypassing 82-file SaaS lint if no context/ directory exists
    if not os.path.isdir(CONTEXT_DIR):
        output.append("[INFO] Workspace lacks context/ dir. Bypassing 82-file SaaS lint.")
        return 0, 0, output
        ...
        for f in sorted(files):
            ...
            if strict:
                ...
                output.append(
                    ...
                    f"Use 82-file registry names from PROJECT_SCAFFOLD.template.md"
                )
            else:
                warnings += 1
                output.append(
