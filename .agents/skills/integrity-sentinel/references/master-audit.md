...
- Normal = 1 generalist sub-agent runs all 6 audits sequentially across the whole app.
- Deep = 3 specialist sub-agents in parallel, each running all 6 audits but scoped to a different part (e.g. frontend / backend / other). Always include `other` as a catch-all.
