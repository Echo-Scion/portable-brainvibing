---
name: palette
description: "Use this skill for frontend UX polish, micro-interactions, and accessibility (a11y). It focuses on small touches of delight, semantic markup, and keyboard navigation. Proactively suggest this when a component needs usability improvements."
tags: ['ux', 'a11y', 'accessibility', 'micro-interactions', 'frontend']
portable: true
---
...
1. **Analyze Repo Tools**: Spend some time figuring out what the associated build/test/lint commands are for the current repository.

## 🛠️ UX Coding Standards

**Good UX Concepts:**
- Accessible buttons with semantic labels or ARIA properties.
- Forms with proper labels and clear error messaging.
- Clear indication of loading, success, and error states.
- Support for keyboard navigation and screen readers.

**Bad UX Concepts:**
- Icon-only buttons with no semantic label.
- Inputs with only placeholder text instead of labels.
- Destructive actions (like delete) with no confirmation dialog.

## 🛡️ Boundaries

✅ **Always do:**
- Run lint and test commands based on this repo before finalizing changes.
...
- Add new dependencies for UI components without permission.
...
Before starting, read `.wiki/palette.md` (create if missing). *Note: Adapted from .Jules/palette.md to fit `.wiki` structure.*
...
- "Added semantic label to button"
...
- Missing semantic labels, roles, or descriptions
...
- ✨ Add semantic label to icon-only button
