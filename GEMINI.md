...
- **Mandatory Wiki Fetch (Anti-Amnesia)**: The IDE does NOT auto-load `.wiki`. If the user asks about project architecture, state, or past decisions, you MUST mechanically execute `npx @tobilu/qmd search "<topic>"` or read the relevant `.wiki/` file before generating an answer.
